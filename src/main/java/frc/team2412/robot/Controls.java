...
public class Controls {
  ...
  public void bindPostClimbControls() {
    climbArmUp
        .and(climbArmDown)
        .and(climbArmDownManual)
        .and(climbArmUpManual)
  ...
  public void bindDriveControls() {
    subsystems.drivebaseSubsystem.setDefaultCommand(
        new DriveCommand(
            ...
            driveController.getLeftYAxis(),
            driveController.getLeftXAxis(),
  ...
  public void bindIndexControls() {
    if (subsystems.intakeSubsystem != null) {
      subsystems.indexSubsystem.setDefaultCommand(
          new IndexCommand(subsystems.indexSubsystem, subsystems.intakeSubsystem));

      subsystems.intakeSubsystem.setDefaultCommand(
          new IntakeCommand(subsystems.intakeSubsystem, subsystems.indexSubsystem));
    }

    shootButton.whileHeld(
        new IndexShootCommand(subsystems.indexSubsystem, subsystems.targetLocalizer));
  }
  ...
  public void bindShooterControls() {
    if (!Subsystems.SubsystemConstants.SHOOTER_TESTING) {
      ...
      driveController
          ...
          .whenPressed(
              new ShooterHoodRPMCommand(subsystems.shooterSubsystem, 1400, 35)
                  .withInterrupt(b)
                  .alongWith(
                      new InstantCommand(() -> subsystems.shooterSubsystem.setTurretAngle(-90))));

      driveController
          .getDPadButton(Direction.LEFT)
          .whenPressed(
              new ShooterHoodRPMCommand(subsystems.shooterSubsystem, 3092, 0)
                  .withInterrupt(b)
                  .alongWith(
                      new InstantCommand(() -> subsystems.shooterSubsystem.setTurretAngle(-90))));
      ...
      driveController
          ...
          .whenPressed(
              new ShooterHoodRPMCommand(subsystems.shooterSubsystem, 0, 0)
                  .withInterrupt(b)
                  .alongWith(
                      new InstantCommand(() -> subsystems.shooterSubsystem.setTurretAngle(0))));
