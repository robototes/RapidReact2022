...
public class GeoConvertor {
  public static Pose2d rigidInchesToPoseMeters(RigidTransform2 trans) {
    return new Pose2d(
        Units.inchesToMeters(trans.translation.x),
        Units.inchesToMeters(trans.translation.y),
        rotation2toRotation2d(trans.rotation));
  }

  public static Pose2d rigidMetersToPoseInches(RigidTransform2 trans) {
    return new Pose2d(
        Units.metersToInches(trans.translation.x),
        Units.metersToInches(trans.translation.y),
        rotation2toRotation2d(trans.rotation));
  }

  public static RigidTransform2 poseToRigid(Pose2d pose) {
    return new RigidTransform2(
        translation2dToVector2(pose.getTranslation()), rotation2dToRotation2(pose.getRotation()));
  }
