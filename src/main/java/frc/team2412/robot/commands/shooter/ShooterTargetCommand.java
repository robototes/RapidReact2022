...
public class ShooterTargetCommand extends CommandBase {
  ...
  public void execute() {
    ...
    else if (turretAngle > ShooterConstants.RIGHT_WRAP_THRESHOLD) state = TurretState.WRAP_RIGHT;
    ...
    switch (state) {
      ...
      case WRAP_LEFT:
        ...
        if (shooter.getTurretAngle() > ShooterConstants.RIGHT_WRAP) state = TurretState.TRACKING;
        break;
      case WRAP_RIGHT:
        turretAngle = ShooterConstants.LEFT_WRAP;
        if (shooter.getTurretAngle() < ShooterConstants.LEFT_WRAP) state = TurretState.TRACKING;
        break;
      case TRACKING:
    ...
    public TurretManager(ShooterSubsystem shooterSubsystem, TargetLocalizer localizer) {
      shooterTargetCommand =
          new ShooterTargetCommand(
              shooterSubsystem, localizer, this::getTurretDisable, this::getIdlePosition);
      idle = 0;
      enabled = false;
    }
