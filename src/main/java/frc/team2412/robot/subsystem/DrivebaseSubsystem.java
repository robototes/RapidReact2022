...
public class DrivebaseSubsystem extends SubsystemBase implements UpdateManager.Updatable {
  ...
  public static class DriveConstants {
    ...
    public static final Rotation2
        PRACTICE_BOT_DRIVE_OFFSET = Rotation2.fromDegrees(-90), // should be 90
  ...
  public DrivebaseSubsystem() {
    ...
    synchronized (sensorLock) {
      gyroscope =
          comp
              ? new PigeonTwo(GYRO_PORT, Hardware.DRIVETRAIN_INTAKE_CAN_BUS_NAME)
              : new NavX(SerialPort.Port.kMXP);
      if (gyroscope instanceof PigeonTwo) gyroscope.setInverted(true);
    ...
    modules =
        new SwerveModule[] {
          FRONT_LEFT_CONFIG.create(supportAbsoluteEncoder),
    ...
    antiTip =
        tab.add("Anti Tip", ANTI_TIP_DEFAULT)
            .withPosition(3, 0)
            .withSize(2, 1)
    ...
    fieldCentric =
        tab.add("Field Centric", FIELD_CENTRIC_DEFAULT)
            .withPosition(3, 2)
            .withSize(2, 1)
            .withWidget(BuiltInWidgets.kToggleSwitch)
            .getEntry();
    ...
    tipController =
        PFFController.ofVector2(TIP_P, TIP_F)
            .setTargetPosition(getGyroscopeXY())
            .setTargetPositionTolerance(TIP_TOLERANCE);

    poseSetX =
        tab.add("Set Pose X", 0.0)
            .withPosition(5, 3)
            .withSize(1, 1)
  ...
  public Vector2 getGyroscopeXY() {
    synchronized (sensorLock) {
      if (gyroscope instanceof PigeonTwo)
        return new Vector2(
                -((PigeonTwo) gyroscope).getAxis(PigeonTwo.Axis.PITCH),
                ((PigeonTwo) gyroscope).getAxis(PigeonTwo.Axis.ROLL))
            .scale(180 / Math.PI);
      if (gyroscope instanceof NavX)
        return new Vector2(
                ((NavX) gyroscope).getAxis(NavX.Axis.ROLL),
  ...
  public void setPose() {
    resetPose(
        new Pose2d(
            poseSetX.getDouble(0.0),
            poseSetY.getDouble(0.0),
  ...
  public Rotation2 getAngle() {
    synchronized (kinematicsLock) {
      return Robot.getInstance().isCompetition()
          ? getPose().rotation.inverse()
          : getPose().rotation;
    }
  }
      ...
      driveSignal =
          new HolonomicDriveSignal(
              translationalVelocity
                  .scale(speedModifier.getDouble(1.0))
                  .rotateBy(getRotationAdjustment()),
              rotationalVelocity * speedModifier.getDouble(1.0),
              false); // changing so drive signal is
      // shuffleboard only
    }
  }
  ...
  // this returns a value that can be rotated to the pose to make the intake the front of the
  // robot
  private Rotation2 getRotationAdjustment() {
    return !Robot.getInstance().isCompetition() ? PRACTICE_BOT_DRIVE_OFFSET : COMP_BOT_DRIVE_OFFSET;
  }
  ...
  public void updateModules(HolonomicDriveSignal driveSignal) {
    ...
    if (driveSignal == null) {
      ...
      chassisVelocity =
          new ChassisVelocity(
              driveSignal.getTranslation().rotateBy(getAngle()), driveSignal.getRotation());
    } else {
      chassisVelocity =
  ...
  public void updateModules(ChassisSpeeds chassisSpeeds) {
    HolonomicDriveSignal holonomicDriveSignal =
        new HolonomicDriveSignal(
            new Vector2(
    ...
    if (trajectorySignal.isPresent()) {
      ...
      synchronized (stateLock) {
        if (getAntiTip() && driveSignal != null) {
          signal =
              new HolonomicDriveSignal( // create updated drive signal
                  accelLimiter
                      .calculate(driveSignal.getTranslation()) // vector accel
                      // limiter
                      .rotateBy(
                          driveSignal.isFieldOriented()
                              ? // flatten
                              getAngle().inverse()
                              : Rotation2.ZERO) // same code as other
                      // block
                      .add(tipController.update(getGyroscopeXY())), // anti tip stuff
                  driveSignal.getRotation(),
                  false); // retain rotation
        } else signal = driveSignal;
      }
    }
