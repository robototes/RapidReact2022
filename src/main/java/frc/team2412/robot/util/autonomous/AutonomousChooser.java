...
public class AutonomousChooser {
  ...
  public AutonomousChooser(Subsystems subsystems) {
    ...
    for (var mode : AutonomousMode.values()) {
      if (mode.enabled) {
        if (!setDefault) {
          ...
          setDefault = true;
        } else autonomousModeChooser.addOption(mode.uiName, mode);
      }
    }
    ...
    setupImage =
        autonomousTab
            .add("Auto Setup", "")
            .withWidget("Image")
            .withProperties(Map.of("Keep Aspect Ratio", true))
            .withSize(5, 3)
            .withPosition(4, 0)
            .getEntry();
    ...
    delayedStartEntry =
        autonomousTab.add("Delay start", 0.0).withWidget(BuiltInWidgets.kNumberSlider).getEntry();
  }
  ...
  public enum AutonomousMode {
    ...
    TWO_BALL_FENDER(
        (subsystems) ->
            new TwoBallFenderAutoCommand(
                subsystems.drivebaseSubsystem, subsystems.shooterSubsystem),
    ...
    JACK_FIVE_BALL(
        (subsystems) ->
            new JackFiveBallAutoCommand(
                subsystems.drivebaseSubsystem,
                subsystems.intakeSubsystem,
                subsystems.indexSubsystem,
                subsystems.shooterSubsystem,
                subsystems.targetLocalizer),
        "Y 2910 Five ball path Y",
        Subsystems.SubsystemConstants.DRIVE_ENABLED
            && Subsystems.SubsystemConstants.INTAKE_ENABLED
            && Subsystems.SubsystemConstants.INDEX_ENABLED
            && Subsystems.SubsystemConstants.SHOOTER_ENABLED,
    ...
    WPI_PATH(
        (subsystems) ->
            new WPILibFiveBallAutoCommand(
                subsystems.drivebaseSubsystem,
                subsystems.intakeSubsystem,
                subsystems.indexSubsystem,
                subsystems.shooterSubsystem,
                subsystems.targetLocalizer),
        "X WPILib Five ball path X",
        Subsystems.SubsystemConstants.DRIVE_ENABLED,
        new Pose2d(new Translation2d(331, 71), Rotation2d.fromDegrees(0)),
        imagesPath + "fiveBall.png"),
    SQUARE_PATH(
        (subsystems) -> new SquarePath(subsystems.drivebaseSubsystem),
        "Square Path",
        Subsystems.SubsystemConstants.DRIVE_ENABLED),
    LINE_PATH(
        (subsystems) -> new LinePath(subsystems.drivebaseSubsystem),
        "Line Path",
        Subsystems.SubsystemConstants.DRIVE_ENABLED),
    STAR_PATH(
        (subsystems) -> new StarPath(subsystems.drivebaseSubsystem),
        "Star Path",
        Subsystems.SubsystemConstants.DRIVE_ENABLED),
    CLIMB(
        (subsystems) -> new ClimbTestCommand(subsystems.climbSubsystem),
        "Climb test",
        Subsystems.SubsystemConstants.CLIMB_ENABLED),
    INDEX(
        (subsystems) -> new IndexTestCommand(subsystems.indexSubsystem),
        "Index test",
        Subsystems.SubsystemConstants.INDEX_ENABLED),
    INTAKE(
        (subsystems) -> new DiagnosticIntakeCommandGroup(subsystems.intakeSubsystem),
        "Intake test",
        Subsystems.SubsystemConstants.INTAKE_ENABLED),
    SHOOTER(
        (subsystems) ->
            new ShooterTurretSetAngleCommand(
                subsystems.shooterSubsystem, subsystems.shooterSubsystem.getTurretTestAngle()),
        "Shooter test",
        Subsystems.SubsystemConstants.SHOOTER_ENABLED),
    CLIMB_DOWN_IN_QUEUE(
        (subsystems) ->
            new ClimbRetractSlowlyCommand(
    ...
    INTAKE_SHOOTER(
        (subsystems) ->
            new IndexShootCommand(subsystems.indexSubsystem)
                .alongWith(
    ...
    private AutonomousMode(
        CommandSupplier commandSupplier, String uiName, boolean enabled, Pose2d startPose) {
      this.commandSupplier = commandSupplier;
      this.uiName = uiName;
      this.enabled = enabled;
      this.startPose = startPose;
