...
public class TargetLocalizer implements Loggable {
  ...
  public TargetLocalizer(
    ...
    this.gyroAdjustmentAngle =
        startingPose.rotation.rotateBy(drivebaseSubsystem.getGyroscopeUnadjustedAngle().inverse());
  }
   ...
   * is to compensate for a longer time of flight the farther away you are and it is not perfect but
   * it should work. angular velocity is to help the turret keep heading when the robot itself is
   * turning
   *
   * @return adjustment
   */
   ...
   * forward from the driver station (Positive rotation is clockwise). If the limelight doesn't have
   * a target, returns {@link RigidTransform2#ZERO}.
   *
   * <p>For example, returning {@code RigidTransform2(Vector2(12, -24), Rotation2.fromDegrees(20))}
   * means that, looking out from the driver station, the limelight is one foot to the right of and
   ...
   *
   * <p>The translation (inches) is relative to the hub, and the rotation is relative to straight
   * forward from the drive station (Positive rotation is clockwise). If the limelight doesn't have
   * a target, returns {@link RigidTransform2#ZERO}.
   *
   ...
   *
   * <p>The translation (inches) is from the starting position, and the rotation is relative to the
   * starting rotation (Positive is clockwise). If the limelight doesn't have a target, returns
   * {@link RigidTransform2#ZERO}.
   *
   * <p>For example, returning {@code RigidTransform2(Vector2(12, -24), Rotation2.fromDegrees(20))}
   * means that, looking from the driver station, the robot moved one foot to the right and two feet
   * closer, and rotated 20 degrees clockwise.
   *
