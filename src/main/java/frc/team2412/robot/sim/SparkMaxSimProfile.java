 ...
 *
 * Using PhysicsSim.addSparkMax() is recommended over manually creating sim profiles and adding them
 * to the sim.
 */
public class SparkMaxSimProfile extends SimProfile {
    ...
    // Note: REVPhysicsSim is supposed to have a singleton, but instantiating separate instances works.
    // Each sim has a separate ArrayList, though, so having separate sims with only one motor is
    // inefficient. The reason to have separate instances for each sim profile is so that running one
    // won't affect others.
